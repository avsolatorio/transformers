...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        file_lock = FileLock("/content/_load_optimizer_and_scheduler.lock")
        logger.info(f"Starting: acquiring lock for self._load_optimizer_and_scheduler(resume_from_checkpoint)")
        with file_lock.acquire(timeout=-1, poll_intervall=random.randint(3, 23)):
            logger.info(f"Starting: lock acquired for self._load_optimizer_and_scheduler(resume_from_checkpoint)")
